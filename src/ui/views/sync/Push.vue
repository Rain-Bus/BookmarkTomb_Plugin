    ...
    <v-stepper v-model="stepFlag">
      ...
      <v-stepper-header>
        ...
        <v-stepper-step
            ...
            step="2"
        >
          Choose Remote Parent Collection
        </v-stepper-step>
...
import {getLocalCollectionTree, getServerCollectionTree, setTreeNode} from "@/lib/util/treeViewUtil";
...
export default class PushComponent extends Vue {
  ...
  async firstStep() {
    ...
    // Need Firstly set the tree to empty due to the Vuetify rebuild statement of tree,
    // if directly income a similar tree, the TreeView will show a bug
    // ---- The selection-type attribute will set to "leaf", and the disabled node will be enabled.
    this.serverTree = []
  ...
  async initSyncItem() {
    ...
    const localCollectionId = this.localSelection[0].id
    const localParentCollectionId = parseInt((await browser.bookmarks.get(localCollectionId.toString()))[0].parentId)
    const serverParentCollectionId = this.serverSelection[0].id
    this.syncItem = new SyncItem(localCollectionId, localParentCollectionId, this.warnConflictCollection, serverParentCollectionId)
    // upsertSyncItem()
    // setTimeout(() => this.finishFlag = false, 1000)
  }
  ...
  detectCollectionExist() {
    if (this.serverSelection.length !== 0 && this.localSelection.length !== 0) {
      let serverChild = this.serverSelection[0].children
      let serverChildNames = serverChild.map(child => child.name)
      let conflictIndex = serverChildNames.indexOf(this.localSelection[0].name)
      if (conflictIndex != -1) {
        this.warnConflictCollection = serverChild[conflictIndex].id
        return true
      }
      this.warnConflictCollection = undefined
    }
    return false
  }
